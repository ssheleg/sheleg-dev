...
- **`error-tracking` carried `metadata.version: 0.9.2` under a release whose every other
  surface said 0.11.5** — the only skill in the member carrying the key, a fifth version
  surface the ×4 sync never read. The key is dropped rather than synced: four surfaces
  move together under a guard, and a fifth existed only to drift.
- **Five skills declared no `license:`** while their two siblings did — `ad-tracking`,
  `crypto-payments`, `frontend-performance`, `google-auth`, `google-signin` all say MIT now.
- **`error-tracking`'s NOT-for clause named `triage-issue`**, an id that resolves only on
  machines carrying one specific tracker plugin; it names the class now.
- **`heleket-provider.md` pointed at a bare `references/price-integrity.md`**, which
  resolves only in the sibling skill; the pointer carries `../stripe-billing/` now.
- **`ad-tracking`'s troubleshooting table moved into `references/performance-security.md`**
  (~285 body tokens), and `stripe-billing`'s cancellation bullets compressed against their
  own reference (~65) — headroom bought by moving depth, never by deleting it. What still
  sits near the ceiling is `B-106` on the board.

One finding was refused, and the refusal is the record: the audit asked for the shared
`webhook signature` literal to be qualified per provider, and `test/validate.py` already
carries `TRIGGERS_SHARED_BY_DESIGN` saying both rails own that seam — the attempted
removal turned the gate red, which is the exemption doing its job.

Board: `B-106` filed. Source: family audit 2026-09-06.

## v0.11.5 — the card is the umbrella's render, and the eyebrow stops painting past the canvas

The committed social preview is now generated by the umbrella with the CORRECTED fit
metric and copied here byte for byte. Nothing about this pack changed.

**Why it was stale.** This member sat in `scripts/site.js`'s `LEGACY_FIT` set, so its card
was rendered with the pre-correction metric and its eyebrow painted to x=1199 — over the
padding box the fix exists to remove. A member's card pixels are committed here and
byte-checked from the umbrella, so only this repository's own release can move them.
