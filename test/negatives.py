...
def main(argv):
    ...
    for name, script, cdir, r in _results:
        ...
        # Case-insensitive, and the fourth thing that differs from the original.
        # `task-pipeline`'s plants print `OK:`; this repository's print `ok:`. Copying
        # the runner verbatim was the right instinct -- one implementation of one idea
        # -- and it carried an assumption about output convention the other repository
        # does not share, so the first local run reported two healthy steps as guards
        # that do not fire while every plant inside them printed `ok`. That is the
        # ALL-44 class arriving inside its own remedy, and it was found by RUNNING
        # rather than by reading. A runner reads what the repository prints; it does
        # not impose a convention on it.
        _out = r.stdout.upper()
        passed = r.returncode == 0 and any(_k in _out for _k in SUCCESS_MARKERS)
