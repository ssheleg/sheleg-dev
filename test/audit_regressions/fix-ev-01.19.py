#!/usr/bin/env python3
"""FIX-EV-01.19 — the outcome corpus for frontend-performance (sherlock audit,
parent FIX-EV-01; depends on the family harness of FIX-EV-01.01).

The corpus (evals/cases/frontend-performance.json) is anchored to the audit's own
...
on ARTIFACTS through the family's outcome-case contract, so frontend-performance can
...
CASES = os.path.join(ROOT, "evals", "cases", "frontend-performance.json")
...
def t_cases_are_structurally_valid():
    ...
    for c in m["cases"]:
        ...
        assert c["skill"] == "frontend-performance"
...
def t_positive_pins_the_field_lab_split():
    c = next(x for x in manifest()["cases"] if "lcp-audit" in x["id"])
    p = c["prompt"]["text"]
    assert "FCP is BOTH" in p, "the DV-17 field/lab correction is not pinned"
    assert "render-blocking" in p and "without dimensions" in p, \
        "the LCP/CLS fixture shapes are not pinned"


def t_negative_refuses_to_load():
    neg = next(c for c in manifest()["cases"] if "negative" in c["id"])
    assert "frontend-performance" in neg["checks"]["load_trace"]["expect_not_loaded"]


def t_diagnostics_and_hostile():
    m = manifest()
    d = next(c for c in m["cases"] if "diagnostics" in c["id"])
    p = d["prompt"]["text"]
    assert "trades" in p and "security or user support for a score is REJECTED" in p, \
        "the DV-18 diagnostics-not-prescriptions outcome is not pinned"
    assert "judged CONDITIONALLY" in p
    ho = next(c for c in m["cases"] if "hostile" in c["id"])
    assert "lab-only" in ho["prompt"]["text"], "the pre-fix defect is not named"
...
def t_live_gated_and_manifest_rules():
    ...
    assert "PERF_AUDIT_URL" in cmd and "PSI_API_KEY" in cmd, \
        "the live case has no host/key probe — it cannot NOT_RUN"
...
def main():
    ...
    case("the positive pins the DV-17 field/lab split", t_positive_pins_the_field_lab_split)
    case("the negative case refuses to load the skill", t_negative_refuses_to_load)
    case("the DV-18 diagnostics and DV-19 hostile cases are pinned",
         t_diagnostics_and_hostile)
    case("the live case is host/key-gated; manifest rules recorded",
