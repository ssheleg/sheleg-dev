#!/usr/bin/env python3
"""FIX-EV-01.16 — the outcome corpus for ad-tracking (sherlock audit,
parent FIX-EV-01; depends on the family harness of FIX-EV-01.01).

The corpus (evals/cases/ad-tracking.json) is anchored to the audit's own
...
on ARTIFACTS through the family's outcome-case contract, so ad-tracking can
...
CASES = os.path.join(ROOT, "evals", "cases", "ad-tracking.json")
...
def t_positive_pins_both_static_checks():
    c = next(x for x in manifest()["cases"] if "consent-gated" in x["id"])
    expects = {o.get("expect") for o in c["checks"]["outcome"]}
    assert "noscript" in expects and "allow_enhanced_conversions" in expects, \
        "the two DV-14 static checks are not pinned"


def t_negative_refuses_to_load():
    neg = next(c for c in manifest()["cases"] if "negative" in c["id"])
    assert "ad-tracking" in neg["checks"]["load_trace"]["expect_not_loaded"]


def t_near_miss_and_hostile():
    m = manifest()
    nm = next(c for c in m["cases"] if "near-miss" in c["id"])
    p = nm["prompt"]["text"]
    assert "NOT automatically required" in p and "Basic consent mode passes" in p, \
        "the DV-13 near-miss is not pinned"
    assert "NOT treated as granted" in p, "unknown jurisdiction may still be granted"
    ho = next(c for c in m["cases"] if "hostile" in c["id"])
    assert "must FAIL both" in ho["prompt"]["text"], "the old output may still pass (DV-19)"
    assert any((o.get("expect") or "") == "release" for o in ho["checks"]["outcome"]), \
        "results are not tied to a release (DV-19)"
...
def main():
    ...
    case("the positive pins both DV-14 static checks", t_positive_pins_both_static_checks)
    case("the negative case refuses to load the skill", t_negative_refuses_to_load)
    case("the DV-13 near-miss and DV-19 hostile cases are pinned",
         t_near_miss_and_hostile)
