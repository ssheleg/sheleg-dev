...
Rows **077 through 081**, 2026-08-31. The audit's four remaining tails (rows DEV-06
through DEV-08 and DEV-10 of its evidence file) — the last undated vendor facts, an
unresolvable reference-implementation pointer, an unpinned library major, and an eval
suite whose own RESULTS.md said "authored, never executed" — plus a dead `$schema`
address a sibling repository's run surfaced (row 081, shipped as v0.11.2).

| REQ | Requirement | Verified by | Result | Observed at | Status |
|---|---|---|---|---|---|
| 077 | The behavioral evals cover all seven skills and stay schema-valid | `python3 test/evals_validate.py`; `python3 test/evals_validate.py --self-test` | Three scenarios authored for the uncovered skills — `crypto-payments` (s04, crediting under hostile deliveries), `error-tracking` (s05, scrubbing layers and release parity), `frontend-performance` (s06, vitals moved and proven) — matching the existing three's shape. `OK: 14 trigger cases and 6 scenarios validate`; the planted invalid trigger class still caught | 2026-08-31 | **verified locally · unreleased** |
| 078 | Every eval has a dated executed run, with the method stated | `test/evals/RESULTS.md`, rows dated 2026-08-31 | 84 trigger probes (14 queries × 3 fresh subagent sessions × 2 models via the Agent tool: haiku, sonnet) and all 24 scenario lines scored against fresh sonnet implementation designs. Haiku: train 23/24, validation 15/18. Sonnet: train 23/24, validation 16/18; scenario lines 23/24 (s01 lacks a refund-path test in its design). Two findings recorded rather than tuned away: q13 ("Explain what LCP means") false-triggers `frontend-performance` in 5 of 6 runs across both models, and one sonnet run routed q02 to `task-pipeline` — coexistence changes routing, as the README warns. The Method section states the limits: harness probes and scored designs, not interactive user sessions | 2026-08-31 | **verified locally · unreleased** |
| 079 | `ad-tracking`'s remaining external facts carry their dates, and were re-verified rather than stamped | `support.google.com/google-ads/answer/13258081` read 2026-08-31; cookie mechanics cross-checked against conversion-linker documentation; `python3 test/validate.py` (body budget) | The console step said `Tools → Conversions → …`, the pre-redesign path — corrected to **Goals → Conversions → Settings → Enhanced conversions** with its date, so DEV-06 closed with a fix rather than a date on a stale claim. `_gcl_aw` was true but incomplete: now dated with the ~90-day default life and the Safari ITP caveat (JS-set cookies capped at 7 days, 24 hours after a link-decorated navigation — which a gclid landing is). Body ~4726 tokens of the 4750 house limit | 2026-08-31 | **verified locally · unreleased** |
| 080 | Every address a skill hands the reader resolves, and the library floor is stated where the examples need it | `grep -rn 0xDEV plugins/` (SKILL.md hit gone); npm registry + `google-auth-library-nodejs` CHANGELOG read 2026-08-31 | `google-signin/SKILL.md` no longer points at the private Prowl repo; the shipped `references/full-guide.md` skeleton is named as the reference implementation (DEV-07 / B-102). `google-auth`'s install block now states that the `client.fetch()` examples require **≥ 10.1.0** — the fetch-compatible API landed in 10.1.0 (2025-06-12, PR #1939), `client.request()` is the 9.x form and survives in 11.x, latest 11.0.2 (DEV-08 / B-101) | 2026-08-31 | **verified locally · unreleased** |
| 081 | Both manifests declare a `$schema` address that resolves, and each names its own contract | `curl -sL -o /dev/null -w '%{http_code}'` against all three schemastore URLs, 2026-08-31 | The declared `claude-code-plugin.json` redirects to a **404** (found by agent-stack's wave-3 run, re-measured here before changing anything); `claude-code-plugin-manifest.json` and `claude-code-marketplace.json` return **200**. `plugin.json` now declares the manifest schema and `marketplace.json` the marketplace schema — the latter had been pointing at the plugin schema even before the address died. Nothing fetches the URL at validate time, which is why every gate stayed green over a dead address | 2026-08-31 | **verified locally · unreleased** |
| 082 | The `$schema` a manifest declares is read at the document ROOT, and a guard refuses the inert position that shipped | `python3 test/validate.py` (check 25); `python3 test/check_schemas.py`; four planted defects via `python3 test/negatives.py` | Row 081 fixed the schema NAME and left the declaration inside `plugins[0]`, where nothing reads it: `json.load(open('.claude-plugin/marketplace.json')).get('$schema')` returned **None** — the marketplace had no effective declaration at all, and the release that closed the defect read exactly like the release that closed it. Root keys before: `['name', 'owner', 'description', 'plugins']`; after: `['$schema', 'name', 'owner', 'description', 'plugins']`, nested copy removed. Both documents now fetch and validate: marketplace → 200 `'Claude Code Plugin Marketplace'`, manifest → 200 `'Claude Code Plugin Manifest'`, `OK: 2 declared schema(s) resolve and validate what declares them`. **Measured while doing it, and it is why the guard pins rather than validates:** the served marketplace schema lists `$schema` among the permitted properties of a plugins[] entry, so the inert declaration was schema-LEGAL and no conformance run could ever have rejected it; and this repository's marketplace checked against the *manifest* schema raises **0** errors (that schema requires only `name`, and neither root sets `additionalProperties: false`), while the manifest against the *marketplace* schema raises **2** ('owner', 'plugins' required) — the wrong schema is silent in the direction that mattered here. Guard shape adopted from `telegram-dev`'s check 14 and `test/check_schemas.py` (its 0263b89, 2026-08-31), whose family-wide sweep found this. `test/validate.py`'s verdict block is now under `if __name__ == '__main__'` so the address map has one home both halves import. Four plants watched failing by name, each the SOLE failure on the copy (`FAIL: 1 problem(s)`) — the first sweep ran while the tree was still red on the unwritten CHANGELOG entry, which makes every plant pass vacuously, so it was re-run after `python3 test/validate.py` reached exit 0; row 064's lesson, re-paid and re-caught. `MIN_EXPECTED` 43 → 47, the value taken from `python3 test/negatives.py --list` rather than typed; verdict 24 → 25 checks. `npm run test:schemas` is the online half's entry point | 2026-08-31 | **verified locally · unreleased** |
