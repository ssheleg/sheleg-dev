...
confirmed against v0.4.3 as it stands on `main` and on npm; nothing was back-filled from
a CHANGELOG entry, because a claim restated is not a claim verified.

---

## Shipped state — v0.4.3, `main` at `33bba49`

Released: `@ssheleg/sheleg-dev@0.4.3` (npm), tag `v0.4.3`.
CI: run `31749477902`, `validate` → `completed success`, 0 failed steps.
...
|---|---|---|---|---|
| 001 | The structural validator passes on the shipped tree | `python3 test/validate.py` | `OK: sheleg-dev structurally valid (12 checks, 6 skill(s), v0.4.3)` — and it counts the skills, so a lost one changes the line | **verified** |
...
| 003 | Version is synchronised across every surface | read back from `package.json`, `.claude-plugin/marketplace.json`, `plugins/sheleg-dev/.claude-plugin/plugin.json`, the top `## vX.Y.Z` in `CHANGELOG.md` | all four → `0.4.3` | **verified** |
...
| 005 | Every reference a skill links resolves, and none is orphaned | walk each `SKILL.md` for `](references/…)` and each `references/*.md` for a mention | **26 links, 0 unresolved; 0 orphan references** across all six skills | **verified** |
...
| 009 | npm serves exactly the version this tree claims | `npm view @ssheleg/sheleg-dev version` | `0.4.3` | **verified** |
| 010 | The tag exists at the released version | `git tag --sort=-v:refname \| head -2` | `v0.4.3`, `v0.4.2` — newest tag matches the shipped version | **verified** |
...
- **The eight negatives, one by one, locally.** REQ-002 reads their step conclusions from
  ...
  reproductions.
- **This repository has no `npm test`.** `package.json` carries an empty `scripts` block,
  so the family's usual one-command gate does not exist here — CI calls
